...
def get_node_features(mol, exclude_node):
    node_features_list = []
    for atom_index, atom in enumerate(mol.GetAtoms()):
        if exclude_node is None or atom_index != exclude_node:
            # 为每个原子获取特征，排除了exclude_node中的原子
            node_features_list.append(feature_ops.atom_featurizer(atom, None))

        # 将原子特征列表转换为NumPy数组
    node_features = np.array(node_features_list, dtype='float32')
...
def smiles2graph(smiles_string,  exclude_node, exclude_edge=None):
    """
    ...
    # bond
    num_bond_features = get_edge_dim(None)  #edge_dim
    if len(mol.GetBonds()) > 0:  # mol has bonds
        ...
        for bond in mol.GetBonds():
            ...
            if i == exclude_node or j == exclude_node:
                continue
...
class SMRTDatasetOneHot(DGLDataset):
    ...
    def process(self):
        ...
        self.graphs, self.label,  self.all_fp= self._load_graph(filename)
...
def load_smrt_data_one_hot(random_state=42,dataset_name=None, raw_dir = "dataset", exclude_node=None, exclude_edge=None):
    set_path = os.path.join(raw_dir,dataset_name)+".csv"
    np.random.seed(random_state)
    df = pd.read_csv(set_path)
    df_shuffled = df.sample(frac=1).reset_index(drop=True)

    train_df, temp_df = train_test_split(df_shuffled, test_size=0.20, random_state=42)
    val_df, test_df = train_test_split(temp_df, test_size=0.5, random_state=42)

    train_df.to_csv(os.path.join(raw_dir,dataset_name)+"_train"+".txt", index=False, sep='\t')
    val_df.to_csv(os.path.join(raw_dir,dataset_name)+"_valid"+".txt", index=False, sep='\t')
    test_df.to_csv(os.path.join(raw_dir,dataset_name)+"_test"+".txt", index=False, sep='\t')

    train_dataset = SMRTDatasetOneHot(name=dataset_name+"_train", raw_dir = raw_dir, exclude_node=exclude_node, exclude_edge=exclude_edge)
    valid_dataset = SMRTDatasetOneHot(name=dataset_name+"_valid", raw_dir = raw_dir, exclude_node=exclude_node, exclude_edge=exclude_edge)
    test_dataset = SMRTDatasetOneHot(name=dataset_name+"_test", raw_dir = raw_dir, exclude_node=exclude_node, exclude_edge=exclude_edge)
...
def load_smrt_data_one_hot_ecfpmix(random_state=42,dataset_name=None, raw_dir = "dataset", exclude_node=None, exclude_edge=None):
    set_path = os.path.join(raw_dir,dataset_name)+".csv"
    np.random.seed(random_state)
    df = pd.read_csv(set_path)
    df_shuffled = df.sample(frac=1).reset_index(drop=True)
    df_shuffled.to_csv(os.path.join(raw_dir,dataset_name)+"_train"+".txt", index=False, sep='\t')
    train_dataset = SMRTDatasetOneHot(name=dataset_name+"_train", raw_dir = raw_dir, exclude_node=exclude_node, exclude_edge=exclude_edge)
    print(f"----len----\n"
          f"len of train_dataset: {len(train_dataset)}\n"
          )
...
class TLDataset(SMRTDatasetOneHot):
    def __init__(self, name, raw_dir="dataset"):
        super(TLDataset, self).__init__(name=name, raw_dir=raw_dir)

    def process(self):
        filename = self.name + ".csv"
        self.graphs, self.label ,_= self._load_graph(filename)
...
class RikenDataset(TLDataset):
    ...
    def _load_graph(self, filename):
        ...
        g_labels = data["RT"]
        ecfps_1024 = data["ECFP_1024"]
        ecfps_2048 = data["ECFP_2048"]
        maccs = data["MACCS"]
        avalon = data["Avalon"]
        erg = data["ErG"]
        all_fp = [ecfps_1024,ecfps_2048,maccs,avalon,erg]
        g_labels = torch.tensor(g_labels, dtype=torch.float32)
        graph_list = []
        for i in range(len(x_smiles)):
            g = feature_to_dgl_graph(smiles2graph(x_smiles[i], exclude_node=self.exclude_node, exclude_edge=self.exclude_edge))
